...
void loop(void) {
  ...
  if (id != 0)
  {
    ...
    Serial.println();
    Serial.println("------------------------------"); 
    Serial.print("Leyendo tarjeta id #");
    Serial.println(id);
    Serial.println("------------------------------"); 
    Serial.println();
#endif
    uint8_t keys[]= { 0xFF,0xFF,0xFF,0xFF,0xFF,0xFF };// default key of a fresh card
    uint8_t metro[]= { 0xXX,0xXX,0xXX,0xXX,0xXX,0xXX };// default key of a fresh card
    for(uint8_t blockn=0;blockn<64;blockn++) 
    {
      //if(nfc.authenticateBlock(1, id ,blockn,KEY_A,keys)) //authenticate block blockn
      if(nfc.authenticateBlock(1, id ,blockn,KEY_A, 0xXX,0xXX,0xXX,0xXX,0xXX,0xXX)) //authenticate block blockn
      {
